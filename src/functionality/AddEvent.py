...
async def add_event(client, message):
    ...
    # splits the users response into an array to be converted into an "Event" object
    event_array = re.split('\s|:|/', event_msg)
    # If a user did not enter "am" or "pm"
    if event_array[3].lower() != "am" and event_array[3].lower() != "pm":
        # a "None" element is inserted in its place
        event_array.insert(3, None)
    if 10 > len(event_array):       # If a description was not included
        # a "None" element is added to its place at the end of the array
        event_array.append(None)

    # Tries to create an Event object from the user input
    try:
        current = Event(event_array[0], int(event_array[1]), int(event_array[2]), event_array[3], event_array[4], int(
            event_array[5]), int(event_array[6]), int(event_array[7]), event_array[8], event_array[9])
